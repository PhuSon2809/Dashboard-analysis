...
;(function () {
  class FinSdk {
    ...
    // #srcSt = "https://st_a.ibe.app:8081";
    // #srcEx = "https://ex_a.ibe.app:8080";
    #srcHandleFile = 'https://commerce_a.ibe.app:8082'
    #srcSt = 'https://finsdk_st.m.pro'
    #srcEx = 'https://finsdk_ex.m.pro'
    // #srcHandleFile = "https://finsdk_frame_ecom.m.pro";
    ...
    async init({ onProgress, onFinish, onError }) {
      // this.#initCSS();
      this.#initFrame({
        onProgress,
        onFinish: async () => {
    ...
    async handleFetchLink(link) {
      try {
        this.srcHandleFile.contentWindow.postMessage(
          {
            command: 'fetchLink',
            value: link
          },
        ...
        console.log('---------------handleFetchLink', error)
      }
    }
    ...
    async #sendMessage({ iframeName, message, isWaitingResponse = true, option }) {
      try {
        ...
        if (iframeName === 'commerce') {
          this.srcHandleFile.contentWindow.postMessage(message, '*')
        }
        if (iframeName === 'dApp') {
          this.dAppFrame.contentWindow.postMessage(message, '*')
        }
        if (typeof this.#listCommand[message.command] === 'undefined') {
          return
        }
    ...
    async postCommerce({ command, value }) {
    ...
    #initFrame({ onProgress, onFinish, onError }) {
      ...
      window.addEventListener('message', async (event) => {
        ...
        const isMessageFromDApp = event.origin === this.dAppHost
        if (command === 'progress') {
          if (data == 100) {
            if (isMessageFromST) {
            ...
            if (isMessageFromEx) {
              ...
              this.srcHandleFile = this.#createFrame({
                id: 'fin_commerce',
                src: `${this.#srcHandleFile}/index.html`,
                onProgress,
                onFinish,
                onError
              })
            ...
            if (this.isSTLoaded && this.isEXLoaded && this.isHandleFileLoaded) {
              onProgress(100)
              onFinish()
            } else {
        ...
        if (isMessageFromDApp) {
          console.log('isMessageFromDApp -->', command)
          const res = await this.#send({
            iframeName: 'ex',
            message: {
              command,
          ...
          console.log('isMessageFromDApp  res -->', res)
          this.#send({
            iframeName: 'dApp',
            message: {
              command,
    ...
    #createFrame({ id, src, styleInline }) {
      this.timeOutLoadIframe[id] = setTimeout(() => {}, 5000)
      const newIframe = document.createElement('iframe')
      newIframe.style = styleInline || 'display: none'
      newIframe.src = src
      newIframe.id = id
    ...
    connectWallet = async (cb) => {
      ...
      setTimeout(() => {
        ...
        elements.forEach((element) => {
          element.onclick = () => {
            this.#walletSelected = element.id.substring(element.id.lastIndexOf('-') + 1)
          }
        })
      }, 300)
    ...
    openDApp = (appInfo) => {
      ...
      this.dAppHost = this.getOrigin(appInfo.url)
      this.dAppFrame = this.#createFrame({
        id: 'd_app_frame',
        src: appInfo.url,
        // src: "https://w.ibe.app:8085",
        // src: "https://commerce_a.ibe.app:8082",
        styleInline:
          'position: fixed; top: 0; left: 0; right: 0; bottom: 0; z-index: 100; width: 400px; height: 600px; background: black'
      })
      console.log('this.dAppFrame --> ', this.dAppFrame)
      this.dAppFrame.onload = (e) => {
        console.log('eeeeee this.dAppFrame', document.getElementById('d_app_frame'))
      }
    }
    sign = async (data) => {
      const res = await this.#send({
    ...
    removeAllWallets = async () => {
      return await this.#sendMessage({
        iframeName: 'st',
        message: {
          command: 'removeAllWallets'
        },
        isWaitingResponse: false
      })
    }
